...
import {
  View, Modal, Text, StyleSheet, TextInput, Animated, Image, StatusBar,
} from 'react-native';
import { GameContext } from '../hook/GameContext';
import RoundButton from './RoundButton';

function PlayersModal({ isVisible, onClose, navigation }) {
  const {
  ...
  const renderContent = () => (
    ...
    <Animated.View style={[styles.modalContent, { transform: [{ scale: scaleValue }] }]}>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'center', marginHorizontal: 30 }}>
        <View style={{
          flex: 1, flexDirection: 'column', justifyContent: 'center', alignItems: 'center', marginHorizontal: 15,
        }}
        >
